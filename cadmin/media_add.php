...
if($dopost=="upload")
{
    ...
    for($i=0; $i<=40; $i++)
    {
        if(isset(${"upfile".$i}) && is_uploaded_file(${"upfile".$i}))
        {
            ...
            // 2011-6-2 ÐÞ¸´¸½¼þÎÞ·¨ÉÏ´«µÄ´íÎó(by:tianya)
            ...
            if (preg_match('#\.(php|pl|cgi|asp|aspx|jsp|php5|php4|php3|shtm|shtml)[^a-zA-Z0-9]+$#i', trim($filename))) { ShowMsg("ä½ æŒ‡å®šçš„æ–‡ä»¶åè¢«ç³»ç»Ÿç¦æ­¢ï¼",'javascript:;'); exit(); } $fullfilename = $cfg_basedir.$filename;
            if($mediatype==1)
            {
    ...
    ShowMsg("³É¹¦ÉÏ´« {$okdd} ¸öÎÄ¼þ£¡","media_main.php");
    exit();
}
