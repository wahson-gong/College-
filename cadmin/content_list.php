...
if($keyword != '')
{
    $whereSql .= " AND ( CONCAT(arc.title,arc.writer,arc.id) LIKE '%$keyword%') ";
}
...
arc.channel,arc.arcrank,arc.click,arc.title,arc.color,arc.litpic,arc.pubdate,arc.mid,t.mobile
FROM  `$maintable` AS arc,dede_addonshop AS t  
$whereSql 
and t.aid=arc.id
...
$dlist->SetParameter('cid', $cid);
$dlist->SetParameter('keyword', $aid);
