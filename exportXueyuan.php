...
if(!empty($_GET['info_sta']))
	$wheres[] = " info_sta = '" . trim($_GET['info_sta']) . "'";
if(!empty($_GET['city']))
...
for($row = 0; $row < count($records); $row++){
	...
	->setCellValue('C'.($excelRow), $records[$row]->city)//城市
	->setCellValue('D'.($excelRow), $records[$row]->requirement)//信息
	->setCellValue('E'.($excelRow), $records[$row]->contact_name1."-".$records[$row]->tel1."或".$records[$row]->contact_name2."-".$records[$row]->tel2)//联系方式
	->setCellValue('F'.$excelRow,$records[$row]->info_fee)//信息费
	->setCellValue('G'.($excelRow), $records[$row]->status)//状态
	->setCellValue('H'.($excelRow), $records[$row]->remark)//备注
	->setCellValue('I'.($excelRow), $records[$row]->date("Y-m-d H:i:s",$records[$row]->trytime))//试讲日期
	->setCellValue('J'.($excelRow), $records[$row]->ac_feedback);//试讲反馈
}
